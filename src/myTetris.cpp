/////////////////////////////////////////////////////////////////////////7
//              LackHack                                     /////////////
//              Christoph Stoppe, 2015                       /////////////
//      based on MakeBlock_WS2812B in Make Magazin 01/2015   /////////////
//////////////////////////////////////////////////////////////////////////
...
boolean my_pause = false; // Variable für Spielunterbrechung (Klomy_pause)
...
void Set(int Spalte, int Zeile, CRGB COLOR) {
    leds( Zeile - 1 + (Spalte - 1) * kMatrixHeight) = CRGB(COLOR);
    //LEDS.show();
}
...
int Pos(int Spalte, int Zeile) {
    int ergebnis = Zeile - 1 + (Spalte - 1) * kMatrixHeight;
    return ergebnis;
}
    ...
    for (int ii = 0; ii < 3; ii++) // Schleife über Brickhöhe
    {
        for (int i = 0; i < 3; i++) // Schleife über Brickbreite
        {
            if (current_brick[ii] & (1 << (i))) {
                if (((Spalte - i) <= 0) or ((Spalte - i) > kMatrixWidth) or ((Zeile + ii) > kMatrixHeight)) belegt = true; // Grenzen des Spielfeldes?
                if (LED_Matrix[Pos(Spalte - i, Zeile + ii)] > 0) //LED_Matrix belegt? 
                {
                    boolean aktuelle_LED_Matrix = false;
                    for (int iii = 0; iii < kMatrixWidth; iii++)
...
void new_Brick() {
    ...
    pos_x = random(brickMeasurement[2], kMatrixWidth - 1 - brickMeasurement[0]); // Zufallsfunktion für Spalte  
    if (Brick[Brick_art][0] > 0) pos_y = 0;
    else pos_y = -1; // Bei einem flachen Brick in der obersten Zeile beginnen
...
void move_down() {
    ...
    my_pause = false; // falls my_Pause dann Spiel fortsetzen
}
...
void flashLine(byte line, byte color) { // ------------Zeile Binken lassen
    ...
    for (int ii = 0; ii < 3; ii++) {
        for (int i = 1; i < kMatrixWidth + 1; i++) Set(i, line, COLOR[0]);
        LEDS.show();
        delay(200);
        for (int i = 1; i < kMatrixWidth + 1; i++) Set(i, line, COLOR[color]);
        LEDS.show();
        delay(200);
    }
}
...
void Zeile_loeschen(int zeile, byte color) {
    ...
    for (int i = (zeile - 1); i > 0; i--)
        for (int ii = 0; ii < kMatrixWidth; ii++) {
            leds(ii * kMatrixHeight + i) = COLOR[LED_Matrix[ii * kMatrixHeight + (i - 1)]]; // kMatrixHeight nach unten verschieben
            LED_Matrix[ii * kMatrixHeight + i] = LED_Matrix[ii * kMatrixHeight + (i - 1)]; // und LED_Matrix speichern 
        }
    for (int i = 0; i < kMatrixWidth; i++) // oberste Zeile löschen
    {
        LED_Matrix[i * kMatrixHeight] = 0;
        leds(i * kMatrixHeight) = CRGB(0, 0, 0); //oberste Reihe ausschalten
    }
    LEDS.show();
}
...
void reihe_voll() {
    for (int i = 0; i < kMatrixHeight; i++) {
        ...
        if (color != 0) {
            for (int ii = 0; ii < kMatrixWidth; ii++) {
                if (LED_Matrix[ii * kMatrixHeight + i] == 0)voll = false;
            }
        } else voll = false;
...
void show_score() {
    LEDS.clear();
    Lauftext_von_rechts("Score:", 2, 100, fg_color, bg_color); // Text, y-Position, Delay, r, g, b   
    Lauftext_von_rechts(String(score), 2, 100, fg_color, bg_color); // Text, y-Position, Delay, r, g, b   
}

void show_highscore() {
    LEDS.clear();
    Lauftext_von_rechts("Highscore:", 2, 100, fg_color, bg_color); // Text, y-Position, Delay, r, g, b   
    Lauftext_von_rechts(String(highscore), 2, 100, fg_color, bg_color); // Text, y-Position, Delay, r, g, b   
}
...
void show_game_over() {
    ...
    LEDS.clear();
    LEDS.setBrightness(Brightness);
    Lauftext_von_unten("GAME OVER", 2, 100, fg_color, bg_color); // Text, y-Position, Delay, r, g, b  
    show_score();
}

boolean Is_Game_Over() {
    int z = 0;
    for (int i = 0; i < kMatrixWidth; i++) if (LED_Matrix[i * kMatrixHeight] > 0) z++;
    if (z > 0) {
...
void new_game() {
    LEDS.clear();
    Lauftext_von_unten("NEW GAME", 2, 100, fg_color, bg_color); // Text, y-Position, Delay, r, g, b
    for (int i = 0; i < NUM_LEDS; i++) {
    ...
    level = 0;
    score = 0;
    Lauftext_blenden("0", 3, 3, 10, fg_color, bg_color); // Text, x-Position, Delay, r, g, b
    new_Brick();
}
...
void check_score() {
    ...
    if (level != tmp_level) {
        ...
        Lauftext_blenden(String(level), 2, 4, 10, fg_color, bg_color); // nächsten Level einblenden
    }
}
...
void setupTetris() {
    ...
    {
        Lauftext_blenden("MAKE", 2, 3, 3, fg_color, bg_color); // Text, x-Position,y-Position, Delay, r, g, b
        Lauftext_blenden(":", 2, 3, 10, fg_color, bg_color); // Text, x-Position,y-Position, Delay, r, g, b
        Lauftext_blenden("BLOCK", 2, 3, 3, fg_color, bg_color); // Text, x-Position,y-Position, Delay, r, g, b
        delay(500);
        Lauftext_blenden("%", 1, 3, 50, fg_color, bg_color); // Text, x-Position, Delay, r, g, b   ==> Smillie
    }
    new_game();
}

void loopTetris() {
    if (my_pause == false) ticks++; // Spieler muss mal aufs Clo ==> drehen und down gleichzeitig gedrückt
    key = get_key(); // Tasten abfragen
    if (game_over == false) { // Aktionen den Tasten zuweisen
        ...
        if (key == 10) my_pause = true; // rotate und move_down gleichzeitig gedrückt = my_Pause 
    } else { // -------- Tasten nach dem Spiel -----
