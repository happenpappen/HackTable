...
void zeile_loeschen(int zeile){
  if (zeile>=0){
    for (int i=0 ; i<kMatrixHeight ; i++) leds[zeile+i*kMatrixWidth]= bg_color;
  }
}
    ...
    for (int spalte=0; spalte<FONT_WIDTH; spalte++)  // 6, 7 od. 8 pixel breite
    {
        for (int zeile=0; zeile<8; zeile++)          // 8 pixel höhe
        {
            if ((spalte+posx>=0) && (spalte+posx<kMatrixHeight)) {
                ...
                {
                    if (((spalte+posx)<=kMatrixHeight) && ((spalte+posx)>=0) ) {leds[posx+spalte+(posy+zeile)*kMatrixHeight-1] = fg_color;}
                } else {
                    if (((spalte+posx)<=kMatrixHeight) && ((spalte+posx)>=0) ) {leds[posx+spalte+(posy+zeile)*kMatrixHeight-1] = bg_color;}
                }
...
{
  if (ch-32>0) zeichnenPos(ASCII[ch-32],x,y, fg_color, bg_color);
}
